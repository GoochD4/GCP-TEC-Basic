...
resource "google_compute_instance" "fortigate" {
  name           = "tec-fgt-${random_string.random_name_post.result}"
...
# Create Ubuntu compute instance
resource "google_compute_instance" "ubuntu" {
  name           = "tec-ubu-${random_string.random_name_post.result}"
  machine_type   = var.machine
  zone           = var.zone
